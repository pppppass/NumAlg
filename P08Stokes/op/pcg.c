...
{
    int n = size, l = level;
    double (*b)[n] = (void*)a_x_u_, (*x)[n] = (void*)u_, (*r)[n] = (void*)w_, (*z)[n] = (void*)(w_+(n-1)*n), (*p)[n] = (void*)(w_+2*(n-1)*n), (*w)[n] = (void*)(w_+3*(n-1)*n), * w_mg_ = w_+4*(n-1)*n;
...
#pragma omp parallel for collapse(2)
    ...
    while (ctr < iter_max)
    {
        // double rho_orig = cblas_dnrm2((n-1)*n, (void*)r, 1);
        double rho_orig = 0.0;
#pragma omp parallel for collapse(2) reduction(+: rho_orig)
        for (int i = 0; i < n-1; i++)
            for (int j = 0; j < n; j++)
                rho_orig += r[i][j]*r[i][j];
        // if (rho_orig < eps)
        if (rho_orig < eps*eps)
            break;
...
//             for (int j = 0; j < n; j++)
//                 z[i][j] = 0.0;
        // cblas_dcopy((n-1)*n, (void*)r, 1, (void*)z, 1);
#pragma omp parallel for collapse(2)
        for (int i = 0; i < n-1; i++)
            for (int j = 0; j < n; j++)
                z[i][j] = r[i][j];
        iter_sol_a_x_mg_gs(n, l, (void*)r, (void*)z, w_mg_, nu_down, nu_up);

        ctr++;
        if (ctr == 1)
        {
            // cblas_dcopy((n-1)*n, (void*)z, 1, (void*)p, 1);
            // rho = cblas_ddot((n-1)*n, (void*)r, 1, (void*)z, 1);
            rho = 0.0;
...
#pragma omp parallel for collapse(2) reduction(+: rho)
            ...
            // cblas_daxpy((n-1)*n, 1.0, (void*)z, 1, (void*)p, 1);
#pragma omp parallel for collapse(2)
            for (int i = 0; i < n-1; i++)
                for (int j = 0; j < n; j++)
...
{
    int n = size, l = level;
    double (*b)[n-1] = (void*)a_y_v_, (*x)[n-1] = (void*)v_, (*r)[n-1] = (void*)w_, (*z)[n-1] = (void*)(w_+n*(n-1)), (*p)[n-1] = (void*)(w_+2*n*(n-1)), (*w)[n-1] = (void*)(w_+3*n*(n-1)), * w_mg_ = w_+4*n*(n-1);
    double rho = 0.0, rho_old = 0.0;
...
#pragma omp parallel for collapse(2)
    ...
    while (ctr < iter_max)
    {
        // double rho_orig = cblas_dnrm2(n*(n-1), (void*)r, 1);
        double rho_orig = 0.0;
#pragma omp parallel for collapse(2) reduction(+: rho_orig)
        for (int i = 0; i < n; i++)
            for (int j = 0; j < n-1; j++)
...
//             for (int j = 0; j < n-1; j++)
//                 z[i][j] = 0.0;
        // cblas_dcopy(n*(n-1), (void*)r, 1, (void*)z, 1);
#pragma omp parallel for collapse(2)
            for (int i = 0; i < n; i++)
                for (int j = 0; j < n-1; j++)
                    z[i][j] = r[i][j];
        iter_sol_a_y_mg_gs(n, l, (void*)r, (void*)z, w_mg_, nu_down, nu_up);

        ctr++;
        if (ctr == 1)
        {
            // cblas_dcopy(n*(n-1), (void*)z, 1, (void*)p, 1);
            // rho = cblas_ddot(n*(n-1), (void*)r, 1, (void*)z, 1);
            rho = 0.0;
#pragma omp parallel for collapse(2) reduction(+: rho)
            for (int i = 0; i < n; i++)
                for (int j = 0; j < n-1; j++)
...
#pragma omp parallel for collapse(2) reduction(+: rho)
            ...
            // cblas_daxpy(n*(n-1), 1.0, (void*)z, 1, (void*)p, 1);
#pragma omp parallel for collapse(2)
            for (int i = 0; i < n; i++)
                for (int j = 0; j < n-1; j++)
                    p[i][j] = z[i][j] + beta * p[i][j];
